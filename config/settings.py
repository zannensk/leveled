...
REFRESH_MS = 2000
SERVER_IDLE_SHUTDOWN_SECONDS = 180  # 3 minutes without heartbeat
DIMENSIONS = ["Math", "Algorithm", "Coding", "Project", "Language"]

# Determine if we are running in PyInstaller bundled mode
def get_base_path():
    if getattr(sys, 'frozen', False):
        # PyInstaller extracts files to sys._MEIPASS; ui/ and config/ are there
        base = getattr(sys, '_MEIPASS', None)
        if base:
            return base
        # Fallback: directory of the EXE itself
        return os.path.dirname(sys.executable)
    # Dev mode: go up two levels from config/settings.py -> project root
    return os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

BASE_DIR = get_base_path()

# Debug helper (printed at startup to verify paths in EXE mode)
if getattr(sys, 'frozen', False):
